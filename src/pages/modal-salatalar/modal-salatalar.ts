...
import { IonicPage, AlertController, Modal, NavController,ModalController, Platform, ViewController, NavParams, Alert } from 'ionic-angular';
...
@Component({
  selector: 'page-modal',
  templateUrl: 'modal-salatalar.html',
}) 

export class ModalSalatalarPage {
  ...
  constructor(
    ...
    // All product information is initially empty. (119 - 225)
    this.characters = [     
      {
        name: '',
        image: '',    
    ...
    // All veggies nutrition information are initially empty. (288 - 310)
    ...
    // All sauces nutrition information are initially empty. (313 - 426)
    ...
    // All names veggies and sauces are initiall empty. (429 - 456)
    this.extras = [
      {       
        veggies1: '',
        veggies2: '',
    ...
    // this is just for making things simple. I used extra instead of extras[0] in modal-salatalar.html
  ...
  // This function starts when the page is loaded. It initially get elements from service2.php and service3.php as json arrays.
  ionViewDidLoad() {      
    this.http.get(this.url2, this.options).map(result => result.json()).subscribe(key => {     
      this.gets = key;
      this.element(this.gets);
      //console.log(global); 
    }, (err) => {
      console.error(err);      
    }   
  ...
  this.http.get(this.url3, this.options).map(result => result.json()).subscribe(key1 => {     
    this.gets1 = key1;
    // To get all salad elements seperately, i added a function. This will divide arrays to their elements. I sended that elements to another function. 
    this.element2(this.gets1);
    //console.log(global); 
    }, (err) => {
    console.error(err);      
    }   
...
// This function gets salad informations from service2.php seperately. 
  element(veri): void {    
    global = veri;     
    // The characters informations were initially empty strings (declared empty in constructor) In this function we are changing their values. We get this information in service2.php.
    // The main reason for dividing this arrays as their elements are we don't want to print all characters. We just want to print the selected salad name and image for example.
    for(var i=0; i<=10;i++) {
      this.characters[i].name = global[i].s_urun_adi;  
      this.characters[i].image = global[i].s_gorsel;  
      this.characters[i].calories = global[i].s_calories;
      this.characters[i].protein = global[i].s_protein;
      this.characters[i].carb = global[i].s_carb;
      this.characters[i].fat = global[i].s_fat;
      this.characters[i].pts = global[i].s_pts;
    } 
  }  
  ...
  Calculate() { 
    ...
    // Summation of total calories of selected salad, selected veggies and selected sauces
    // Summation of total protein of selected salad, selected veggies and selected sauces
    // Summation of total carb of selected salad, selected veggies and selected sauces
    // Summation of total fat of selected salad, selected veggies and selected sauces
    // Summation of total pts of selected salad, selected veggies and selected sauces
  

    this.total_calories = +this.character.calories + +this.veggies_total_calories + +this.sauces_total_calories;
    this.total_protein = +this.character.protein +  +this.veggies_total_protein + +this.sauces_total_protein;
    this.total_carb = +this.character.carb +  +this.veggies_total_carb + +this.sauces_total_carb;
    this.total_fat = +this.character.fat +  +this.veggies_total_fat + +this.sauces_total_fat;
    this.total_pts = +this.character.pts +  +this.veggies_total_pts + +this.sauces_total_pts;
